...
import { useAddPricingGroup } from "../hooks";
import { t } from "i18next";
import { useState } from "react";
import { NumericFormat } from 'react-number-format';
export default function Component(props: any) {
    const { openModal, setOpenModal, setTriggerAdd } = props;
    const { product, productList, price, currency, currencyList, pricingGroupName, errors, register, handleSubmit, handlePriceChange, handleCurrencyChange, handleChangeProduct, addNewDataPricingGroup, handlePricingGroupNameChange } = useAddPricingGroup(setTriggerAdd);
    const handleCloseAddModal = () => setOpenModal({ ...openModal, addModal: false });
    const [addLoading, setAddLoading] = useState(false)

    const onSubmit = async () => {
        try {
            setAddLoading(true)
            await addNewDataPricingGroup(openModal, setOpenModal);
        } catch (error) {
                    ...
                    <form onSubmit={handleSubmit(() => onSubmit())}>
                        ...
                        >
                            ...
                            >
                                ...
                                <Box sx={{ width: "70%" }}>
                                    <Autocomplete
                                        ...
                                        renderInput={(params) => (
                                            <TextField
                                                data-testid="pricing-group-addpricinggroup-productname-autocomplete"
                                                {...params}
                            ...
                            >
                                ...
                                <Box sx={{ width: "70%" }}>
                                    <TextField
                                        data-testid="pricing-group-addpricinggroup-pricinggroupname-text"
                                        label={t("placeholder.pricing-group-name")}
                                        size="small"
                                        autoComplete="current-pricing-group-name"
                            ...
                            >
                                ...
                                <Box sx={{ width: "70%" }}>
                                    <Box sx={{
                                        ...
                                        <NumericFormat
                                            data-testid="pricing-group-addpricinggroup-price-text"
                                            value={price}
                                        ...
                                        >
                                            ...
                                            <Select
                                                data-testid="pricing-group-addpricinggroup-currency-select"
                        ...
                        <Box sx={{ display: "flex", justifyContent: "flex-end", marginTop: "10px" }}>
                            <Button
                                data-testid="pricing-group-addpricinggroup-cancel-button"
                                onClick={handleCloseAddModal}
                                variant="text"
                                children={t("button.cancel")}
                            />
                            <Button data-testid="pricing-group-addpricinggroup-save-button" type="submit" variant="text" children={t("button.save")} disabled={addLoading} />
                        </Box>
                    </form>
                </>
            </ModalCustom>
...
Component.displayName = "AddPricingGroupModal";