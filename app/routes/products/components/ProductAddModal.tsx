...
import { ValidJSON } from "@/core/utils/index.js";
...
export default function Component(dataAdd: any) {
  ...
  const handleFormatWinLoseJSON = () => {
    ...
    if (ValidJSON(winLoseSnapshot)) {
      setSuccess(true);
      const dataFormat = JSON.parse(winLoseSnapshot);
      const result = JSON.stringify(dataFormat, null, 2);
      setWinLoseSnapshot(result); // Format and set JSON if it's valid
    } else {
      setSuccess(false);
  ...
  const onSubmitHandlerAdd = async (data: productProps) => {
    try {
      const dataAddProduct = {
        ...
        wlsnapshot_data: winLoseSnapshot !== '' ? JSON.parse(winLoseSnapshot) : JSON.parse('{}'),
        type: data.type as string
      };
      description
        ? Object.assign(dataAddProduct, { description: description })
        : null;
      const res = await createProduct(dataAddProduct);
      setAddProduct(res);
  ...
  const navigateToSubProduct = (id: any, name: string) =>
    navigate(`${location.pathname}/sub-product`, { state: { id, name } });
            ...
            >
              <Box sx={{
                ...
                <Box sx={{ display: "flex", gap: "1rem" }}>
                  <TextField
                    data-testid="product-addproduct-productname-text"
                  ...
                  />
                  <TextField
                    data-testid="product-addproduct-productname-preview-text"
                ...
                <Box sx={{ display: "flex", gap: "1rem" }}>
                  <FormControl sx={{ width: "48.7%" }}>
                    ...
                    <Select
                      data-testid="product-addproduct-status-select"
                  ...
                  <FormControl sx={{ width: "48.7%" }}>
                    ...
                    <Select
                      data-testid="product-addproduct-type-select"
                      labelId="product-addproduct-type-select-label"
                      id="product-addproduct-type-select"
                      value={productType}
                ...
                {profile?.username == "superadmin" && productType != ProductMasterType.OTHER &&
                  <Box sx={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                    ...
                    <Textarea
                      data-testid="product-addproduct-jsonw/lsnapshot-textarea"
                      style={{ width: "100%" }}
                      minRows={7}
                      aria-label="minimum height"
                      placeholder="Enter JSON format"
                      value={winLoseSnapshot}
                      onChange={(event) => setWinLoseSnapshot(event.target.value)}
                    />
                    <Box sx={{ display: "flex", flexDirection: "row", justifyContent: "end" }}>
                      <Button
                        ...
                        disabled={!winLoseSnapshot ? true : false}
                        data-testid="product-addproduct-process-button"
                        variant="contained"
              ...
              >
                <Button
                  data-testid="product-addproduct-cancel-button"
                  variant="text"
                  children={t("button.cancel")}
                  onClick={handleCloseAddModal}
                />
                <Button
                  data-testid="product-addproduct-save-button"
                  type="submit"
                  variant="text"
...
Component.displayName = "ProductAddModal";
