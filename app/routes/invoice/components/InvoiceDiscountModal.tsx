...
export default function InvoiceDiscountModal(props: DiscountProps) {
    const { t } = useTranslation()
    const { TranslateErrorCode } = useTranslateErrorCode()
    const { alertError, alertSuccess } = useAlertDialog();
    const { openModal, closeModal, discountData } = props
    ...
    const handleDeleteClick = (id: GridRowId, row: any) => () => {
        ...
        handleDelete(discountData.id, row.id)
    };
    ...
    const processRowUpdate = (newRow: GridRowModel) => {
        ...
        console.log('ID : ', discountData.id)
        console.log('row ID : ', id)
        newRow.value = newRow.value || 0
        if (newRow.isNew) {
            handleAdd(discountData.id, id, newRow)
        } else {
            handleUpdate(discountData.id, id, newRow)
        }
    ...
    const handleAdd = async (id: string, no: string, row: GridRowModel) => {
        setLoading(true)
        try {
            const response = await new InvoiceDataService().addInvoiceDiscount(id, no, row)
    ...
    const handleUpdate = async (id: string, no: string, row: GridRowModel) => {
        setLoading(true)
        try {
            const response = await new InvoiceDataService().updateInvoiceDiscount(id, no, row)
            setRows(response.data ? response.data : [])
            alertSuccess(TranslateErrorCode(response.code));
            return response.data
        } catch (error: any) {
    ...
    const handleDelete = async (id: string, no: string) => {
        setLoading(true)
        try {
            const response = await new InvoiceDataService().deleteInvoiceDiscount(id, no)
            setRows(response.data ? response.data : [])
            alertSuccess(TranslateErrorCode(response.code));
            return response.data
        } catch (error: any) {
    ...
    const getDiscount = async () => {
        setLoading(true)
        try {
            const response = await new InvoiceDataService().getInvoiceDiscount(discountData.id)
